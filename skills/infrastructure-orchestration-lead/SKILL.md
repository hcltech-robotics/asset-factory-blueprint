...
description: Route runtime work across local GPU, containers, clusters and storage.
version: 0.1.0
license: MIT
tools:
...
## Purpose
Route runtime work across local GPU, containers, clusters and storage.
...
## Eval coverage
Benchmark and eval payloads for this skill live in the asset-factory-verification repository under skill-checks/infrastructure-orchestration-lead.
Run them from that repository against a blueprint checkout.
Eval cases cover the infrastructure contract path with structured output and explicit validation status.
