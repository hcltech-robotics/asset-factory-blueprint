...
description: Identify available capabilities and tools, select primaries over fallbacks and install what a run needs on demand.
version: 0.1.0
license: MIT
tools:
...
## Eval coverage
Benchmark and eval payloads for this skill live in the asset-factory-verification repository under skill-checks/capability-steward.
Run them from that repository against a blueprint checkout.
Eval cases cover the capability stewardship contract path with structured output and explicit validation status.
